...
let aiProcessedData = null;   // 新增：AI处理后的数据
...
function initializeDropZones() {
    ...
    dropZones.forEach((dropZone, index) => {
        ...
        // 跳过由Ai_analysis.js处理的AI预处理区域
        if (fileType === 'ai-preprocess') {
            console.log('跳过AI预处理区域，该区域由Ai_analysis.js处理');
            return;
        }
...
function handleFileSelection(dropZone, file) {
    ...
    } else if (fileType === 'ai-preprocess') {
        fileInfoId = 'ai-preprocess-file-info';
    } else {
    ...
    const statusId = dropZone.dataset.status;
        ...
        if (isExcel) {
            // 对于AI预处理类型，由AI_analysis.js处理，不需要app.js中的processUploadedFile函数来处理
            if (fileType === 'ai-preprocess') {
                // AI处理文件在Ai_analysis.js中已经处理，这里不需要额外处理
                console.log('AI预处理文件已选择，等待处理');
            } else {
...
function handleFileSelect(e) {
    ...
    if (files.length > 0) {
        ...
        // 通过输入ID判断文件类型
        if (inputId === 'sales-file') {
            type = 'sales';
        } else if (inputId === 'schedule-file') {
            type = 'schedule';
        } else if (inputId === 'anchor-monthly-file') {
            type = 'anchor-monthly';
        } else if (inputId === 'ai-preprocess-file') {
            type = 'ai-preprocess';
        } else {
            type = 'unknown';
        }
        ...
        if (isExcel) {
            // 对于AI预处理类型，由Ai_analysis.js处理
            if (type === 'ai-preprocess') {
                // AI预处理文件在Ai_analysis.js中已经处理，这里不需要额外处理
                console.log('AI预处理文件已选择，等待处理');
            } else {
...
function startAnalysis() {
    ...
    requestAnimationFrame(() => {
        try {
            ...
            // 触发分析完成事件，以激活AI助手
            console.log('分析完成，触发analysis-complete事件');
            document.dispatchEvent(new Event('analysis-complete'));
...
function showNotice(type, message) {
    ...
    const existingNotices = noticeArea.querySelectorAll('.alert');
...
function showNotification(message, type = 'info') {
    ...
    for (let i = 0; i < existingNotifications.length; i++) {
        ...
        const existingText = existingNotification.textContent.trim();
        // 如果已有相同内容的通知，不再创建新通知
        if (existingText.includes(message)) {
            console.log('已存在相同内容的通知，跳过显示:', message);
