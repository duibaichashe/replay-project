...
let processFileData = null;  // 新增：待处理文件数据
let processResultData = null;  // 新增：文件处理结果
...
const exportProcessBtn = document.getElementById('export-process-btn');
const modalExportProcessBtn = document.getElementById('modal-export-process-btn');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 新增：为处理文件输入框添加事件监听
    if (processFileInput) {
        processFileInput.addEventListener('change', handleFileSelect);
        console.log('已为处理文件输入框添加事件监听器');
    }
    ...
    // 新增：绑定文件处理按钮事件
    if (processBtn) {
        processBtn.addEventListener('click', startFileProcessing);
        console.log('已为处理按钮添加事件监听器');
    }

    // 新增：绑定处理结果导出按钮事件
    if (exportProcessBtn) {
        exportProcessBtn.addEventListener('click', exportProcessResult);
        console.log('已为处理结果导出按钮添加事件监听器');
    }

    // 新增：绑定模态框内导出按钮事件
    if (modalExportProcessBtn) {
        modalExportProcessBtn.addEventListener('click', exportProcessResult);
        console.log('已为模态框导出按钮添加事件监听器');
    }
...
function handleFileSelect(event) {
    try {
        ...
        const fileType = fileInput.id.split('-')[0];
        console.log(`选择了 ${fileType} 类型的文件:`, file.name);
        ...
        reader.onload = function(e) {
            ...
            if (fileType === 'sales') {
                salesFileData = {
                    workbook: workbook,
                    rawData: parseSalesExcel(workbook)
                };
                console.log('销售数据已解析:', salesFileData.rawData.length);
                checkEnableAnalyzeButton();
            } else if (fileType === 'schedule') {
                scheduleFileData = {
                    workbook: workbook,
                    rawData: parseScheduleExcel(workbook)
                };
                console.log('排班表数据已解析:', scheduleFileData.rawData);
                checkEnableAnalyzeButton();
            } else if (fileType === 'anchor-monthly') {
                anchorMonthlyData = {
                    workbook: workbook,
                    rawData: parseAnchorMonthlyExcel(workbook)
                };
                console.log('主播月度资料已解析');
                checkEnableAnalyzeButton();
            } else if (fileType === 'process') {
                // 新增：处理文件数据解析
                processFileData = {
                    workbook: workbook,
                    rawData: parseProcessFile(workbook),
                    fileName: file.name
                };
                console.log('处理文件已解析:', processFileData.rawData.length);
                // 启用处理按钮
                enableProcessButton();
            }
        };
        ...
        showNotice('danger', '处理文件时出错: ' + error.message);
    }
}
...
// 新增：解析待处理文件
function parseProcessFile(workbook) {
    try {
        console.log('开始解析待处理文件...');
        const firstSheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[firstSheetName];
        ...
        console.log(`待处理文件解析完成，包含 ${jsonData.length} 行数据`);
        return jsonData;
    } catch (error) {
        console.error('解析待处理文件时出错:', error);
        showNotice('danger', '解析文件时出错: ' + error.message);
        return [];
    }
}
...
function enableProcessButton() {
    if (processFileData && processFileData.rawData && processFileData.rawData.length > 0) {
        if (processBtn) {
...
async function startFileProcessing() {
    try {
        ...
        // 检查文件数据
        if (!processFileData || !processFileData.rawData || processFileData.rawData.length === 0) {
            showNotice('warning', '请先上传文件');
            return;
        }
        
        // 获取处理要求
        const requirements = document.getElementById('process-requirements').value.trim();
        if (!requirements) {
            showNotice('warning', '请输入处理要求');
            return;
        }
        
        // 显示处理进度模态框
        showProcessingModal(requirements);
        
        // 准备AI处理请求数据
        const requestData = {
            requirements: requirements,
            fileData: processFileData.rawData,
            fileName: processFileData.fileName
        };
        
        // 调用AI处理函数
        processResultData = await processFileWithAI(requestData);
        
        // 显示处理结果
        displayProcessResult(processResultData);
        
        // 启用导出按钮
        enableExportProcessButton();
        
    } catch (error) {
        console.error('文件处理时出错:', error);
        showNotice('danger', '文件处理失败: ' + error.message);
        hideProcessingSpinner();
    }
}

// 新增：显示处理进度模态框
function showProcessingModal(requirements) {
    try {
        // 设置处理要求
        const reqElement = document.getElementById('result-requirements');
        if (reqElement) {
        ...
        const resultContainer = document.getElementById('process-result-container');
        
        if (spinner) spinner.classList.remove('d-none');
        if (resultContainer) resultContainer.classList.add('d-none');
        
        // 显示模态框
        const modal = new bootstrap.Modal(document.getElementById('process-result-modal'));
        modal.show();
    } catch (error) {
        console.error('显示处理模态框时出错:', error);
    }
}
...
// 新增：使用AI处理文件
async function processFileWithAI(requestData) {
    // 模拟AI处理
    return new Promise((resolve) => {
        console.log('AI正在处理文件...', requestData);
        
        // 模拟处理延迟
        setTimeout(() => {
            try {
                // 获取原始数据
                const rawData = requestData.fileData;
                ...
                // 示例处理逻辑 - 这里可集成实际AI接口
                if (requirements.includes('筛选')) {
                    // 模拟筛选 - 这里只是简单示例
                    const numRegex = /大于\s*(\d+)/;
                    const match = requirements.match(numRegex);
                    
                    if (match && match[1]) {
                        const threshold = parseInt(match[1]);
                        console.log(`筛选条件: 金额大于 ${threshold}`);
                ...
                // 示例 - 按主播分组
                if (requirements.includes('分组') && requirements.includes('主播')) {
                    // 查找可能的主播列
                    const anchorColumns = columns.filter(col => 
                        col.includes('主播') || 
                        col.includes('播') || 
                        col.includes('名称')
                    );
                    ...
                    if (anchorColumns.length > 0) {
                        const anchorColumn = anchorColumns[0];
                        
                        // 按主播分组并计数
                        const groups = {};
                        processedData.forEach(row => {
                            const anchor = row[anchorColumn] || '未知';
                            if (!groups[anchor]) {
                                groups[anchor] = [];
                            }
                            groups[anchor].push(row);
                        });
                        
                        // 为每组添加组标识
                        processedData = Object.entries(groups).flatMap(([anchor, rows]) => {
                            return rows.map(row => ({
                                ...row,
                                '所属组': anchor,
                                '组内计数': rows.length
                ...
                resolve({
                    processedData: requestData.fileData,
                    summary: {
                        totalRows: requestData.fileData.length,
                        processedRows: requestData.fileData.length,
                        requirements: requestData.requirements,
                        error: error.message
                    }
                });
...
// 新增：显示处理结果
function displayProcessResult(resultData) {
    try {
        // 隐藏加载动画
        hideProcessingSpinner();
        ...
        if (summary) {
            const totalRows = resultData.summary.totalRows;
            const processedRows = resultData.summary.processedRows;
            
            summary.textContent = `处理完成。原始数据 ${totalRows} 行，处理结果 ${processedRows} 行。`;
            
            if (resultData.summary.error) {
                summary.textContent += ` 警告: ${resultData.summary.error}`;
            }
        }
        
        // 生成结果表格
        generateResultTable(resultData.processedData);
        
    } catch (error) {
        console.error('显示处理结果时出错:', error);
        showNotice('danger', '显示处理结果失败: ' + error.message);
    }
}
...
function generateResultTable(data) {
    try {
        ...
        const table = document.getElementById('process-result-table');
        if (!table) return;
        ...
        if (data.length > 100) {
            ...
            infoCell.colSpan = fields.length;
            infoCell.className = 'text-center text-muted';
            infoCell.textContent = `显示了前100行，共 ${data.length} 行数据。导出可获取完整数据。`;
            infoRow.appendChild(infoCell);
            tbody.appendChild(infoRow);
        }
        
    } catch (error) {
        console.error('生成结果表格时出错:', error);
    }
}

// 新增：启用导出处理结果按钮
function enableExportProcessButton() {
    const btnInPage = document.getElementById('export-process-btn');
    const btnInModal = document.getElementById('modal-export-process-btn');
    
    if (btnInPage) {
        btnInPage.classList.remove('d-none');
    }
...
// 新增：导出处理结果
function exportProcessResult() {
    try {
        console.log('导出处理结果...');
        
        if (!processResultData || !processResultData.processedData) {
            showNotice('warning', '没有可导出的处理结果');
            return;
        }
        ...
        // 将JSON数据转换为工作表
        const ws = XLSX.utils.json_to_sheet(processResultData.processedData);
        
        // 添加工作表到工作簿
        XLSX.utils.book_append_sheet(wb, ws, '处理结果');
        
        // 生成文件名 - 使用原始文件名加上后缀
        const originalName = processFileData.fileName || 'processed-file';
        const fileName = originalName.replace(/\.xlsx?$/, '') + '-处理结果.xlsx';
        
        // 导出Excel文件
        XLSX.writeFile(wb, fileName);
        
        showNotice('success', '处理结果已导出');
        
    } catch (error) {
        console.error('导出处理结果时出错:', error);
...
async function startFileProcessing() {
    try {
        ...
        // 调用AI处理函数
        const processedData = await processFileWithAI(processFileData.rawData, requirements);
        
        // 更新处理结果
        processResultData = processedData;
        
        // 显示处理结果
        displayProcessResult();
    } catch (error) {
        console.error('文件处理出错:', error);
        showNotice('danger', '文件处理出错，请重试');
    }
}
...
function displayProcessResult() {
    ...
    if (!processResultData || processResultData.length === 0) {
        resultContainer.innerHTML = '<div class="alert alert-info">无处理结果</div>';
        return;
    }
    ...
    const headerRow = document.createElement('tr');
    processResultData[0].forEach(header => {
        const th = document.createElement('th');
    ...
    processResultData.slice(1).forEach(row => {
        const tr = document.createElement('tr');
        row.forEach(cell => {
            const td = document.createElement('td');
            td.textContent = cell;
...
function exportProcessResult() {
    if (!processResultData || processResultData.length === 0) {
        showNotice('warning', '无处理结果可导出');
        return;
    }
    
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet(processResultData);
    XLSX.utils.book_append_sheet(wb, ws, '处理结果');
    
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '处理结果.xlsx');
}
